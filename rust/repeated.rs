...
/// APPROACH:
/// converge on single cpp.rs trait to impl RepeatedField around, common to
/// Message _and_ Scalar use same trait for upb? mostly for consistency
/// ... generated cpp code has different methods for scalar vs message vs string
///     but RepeatedPtrField and RepeatedField and upb_Array all implement
///         Push(elem)
///     maybe just implement both push_default and push for everything
///
/// push can be implemented with push_default + set + len
/// how does push(Message) ownership work? seems like we may want to expose
/// option of fusing / cloning to users
/// fusing requires transferring ownership of message, (deep-)cloning does not

//TODO: rename this trait
// this trait will be implemented manually on RepeatedField<T>, then blanket
// impls over this trait are used on the final wrappers (RepeatedView/Mut)
// TODO: should bounds checking be part of this trait? or mark unsafe?
trait RepeatedI<'msg, T: Proxied> {
    fn len(&self) -> usize;
