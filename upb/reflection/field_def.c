...
upb_FieldDef* _upb_FieldDefs_New(upb_DefBuilder* ctx, int n,
  ...
  for (int i = 0; i < n; i++) {
    ...
    /*
    const UPB_DESC(FeatureSet*) field_parent_features = parent_features;
    // For fields in a oneof, inherit features from that oneof.
    if (UPB_DESC(FieldDescriptorProto_has_oneof_index)(protos[i])) {
      int oneof_index = UPB_DESC(FieldDescriptorProto_oneof_index)(protos[i]);
      if (oneof_index >= 0 && oneof_index < upb_MessageDef_OneofCount(m)) {
        field_parent_features =
            upb_OneofDef_ResolvedFeatures(upb_MessageDef_Oneof(m, oneof_index));
      }
    }

    _upb_FieldDef_CreateNotExt(ctx, prefix, field_parent_features, protos[i], m,
                               f);
    */
    f->index_ = i;
    if (!ctx->layout) {
